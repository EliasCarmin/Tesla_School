		...
		if ( $current_school = $user_info['current_school'] ) {
			...
			if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
				...
				if ( WLSM_M_Role::check_permission( array( 'manage_admins', 'manage_roles', 'manage_employees' ), $permissions ) ) {
					...
					$school_staff_group_administrator_menu = add_menu_page( esc_html__( 'Administrator', 'school-management-system' ), esc_html__( 'Administrador', 'school-management-system' ), 'read', WLSM_MENU_STAFF_ADMINISTRATOR, array( 'WLSM_Menu', 'school_staff_group_administrator' ), 'dashicons-businessman', 33 );
